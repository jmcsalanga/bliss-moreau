...
circ = visual.Circle(win=win, radius=10, units='pix',lineWidth=1, 
    lineColor=[-1,-1,-1], fillColor=[-1,-1,-1], fillColorSpace='rgb',
    opacity=1, depth=0.0, interpolate=True)
mouse = event.Mouse(win=win)
x, y = [None, None]
mouse.mouseClock = core.Clock()
msg = visual.TextStim(win, text='', pos=(0, 0), color="white")
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        # *circ* updates
        if t >= 0.0 and circ.status == NOT_STARTED:
            # keep track of start time/frame for later
            circ.tStart = t
        ...
        if circ.contains(mouse) and fixationStart == 0.0:
            msg.text = 'inside %1.1f' % totalFixationTime
            fixationStart = fixationClock.getTime()
        elif circ.contains(mouse) and fixationStart > 0.0:
            msg.text = 'inside %1.1f' % totalFixationTime
            totalFixationTime = fixationClock.getTime() - fixationStart
            if totalFixationTime >= 2.0:
                continueRoutine = False
        elif not circ.contains(mouse):
            fixationStart == 0.0
            totalFixationTime = 0.0
            fixationClock.reset()  # clock
    ...
    circ.setPos(Locations)
    # setup some python lists for storing info about the mouse_2
    gotValidClick = False  # until a click is received
    mouse_2.mouseClock.reset()
    # keep track of which components have finished
    trialComponents = [image, image_2, circ, text_2, mouse_2]
    ...
    while continueRoutine:
        ...
        # *circ* updates
        if t >= 0.525 and circ.status == NOT_STARTED:
            # keep track of start time/frame for later
            circ.tStart = t
            circ.frameNStart = frameN  # exact frame index
            circ.setAutoDraw(True)
